...
public:
    static void layout_flex_container(const std::shared_ptr<LayoutBox>& container, const core::RectF& containing_block) {
        ...
        for (auto& child : container->children) {
            ...
            if (is_row) {
                float w = child->style.width.is_auto() ? 120.0f : child->style.width.resolve(container->dimensions.content.width);
                info.hypothetical_main_size = w;
                float h = child->style.height.is_auto() ? (container->dimensions.content.height > 0 ? container->dimensions.content.height : 40.0f)
                                                        : child->style.height.resolve(container->dimensions.content.height);
                info.target_cross_size = h;
            } else {
                float h = child->style.height.is_auto() ? 40.0f : child->style.height.resolve(container->dimensions.content.height);
