#include "../include/nexus/nexus_engine.hpp"
#include <cassert>
#include <iostream>

void test_html_parser() {
    std::cout << "[Test] Running HTML5 Parser & DOM tests..." << std::endl;
    std::string html = "<div id=\"main\" class=\"container hero\"><p>Hello Nexus</p></div>";
    nexus::html::HTMLParser parser(html);
    ...
    assert(ps.size() == 1);
    assert(ps[0]->get_text_content() == "Hello Nexus");
...
void test_css_specificity() {
    ...
    nexus::css::Specificity inline_spec(1, 0, 0, 0);
    nexus::css::Specificity id_spec(0, 1, 0, 0);
    nexus::css::Specificity class_spec(0, 0, 1, 0);
    nexus::css::Specificity elem_spec(0, 0, 0, 1);
    ...
    nexus::css::CSSParser parser(css);
...
void test_box_model_and_flexbox() {
    ...
    nexus::NexusBrowserEngine engine(800, 600);
    std::string html = R"(
...
void test_rasterizer() {
    ...
    nexus::paint::SoftwareRasterizer rasterizer(100, 100, nexus::core::Color::white());
    rasterizer.fill_rounded_rect(nexus::core::RectF(10, 10, 80, 80), nexus::core::BorderRadius(8.0f), nexus::core::Color::blue());
...
int main() {
    std::cout << "========================================\n"
              << "  NEXUS BROWSER ENGINE TEST SUITE       \n"
              << "========================================\n";
    test_html_parser();
