...
public:
    ...
    void draw_text(const std::string& text, float x, float y, float font_size, int font_weight, const core::Color& color) {
        float cx = x;
        for (char c : text) {
            FontRasterizer::render_char(framebuffer_, width_, height_, c, cx, y, font_size, font_weight, color);
            cx += layout::TextShaper::estimate_char_width(c, font_size, font_weight);
        }
    }
