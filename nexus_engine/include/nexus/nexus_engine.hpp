...
public:
    NexusBrowserEngine(int width = 1000, int height = 800)
    ...
    RenderResult render_page(const std::string& html_code, const std::string& custom_css = "", const std::string& js_code = "") {
        ...
        if (!js_code.empty()) {
            core::ScopedTimer timer(result.profiler, "JS_Execution", "Execute ECMAScript runtime & events");
